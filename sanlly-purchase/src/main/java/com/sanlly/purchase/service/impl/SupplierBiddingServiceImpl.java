...
/**
 * @Description: TODO
 * @ClassName: SupplierBiddingServiceImpl
 * @Version 1.0
 * @Author mqz
 */
@Service
public class SupplierBiddingServiceImpl extends BaseServiceImpl implements SupplierBiddingService {
    // 竞价采购
    @Autowired
...
// 竞价采购
        ...
        for (MatterTenderPurchase matterTenderPurchase : matterTenderPurchases) {
            ...
            for (MatterTenderProduct matterTenderProduct : matterTenderProducts) {
                //System.out.println("matterTenderProduct.getProduct()：" + matterTenderProduct.getProduct());
                ...
                if (rsSparePart != null) {
                    ...
                    //System.out.println("rsSparePart.getSparePartsCategoriesDetails():" + rsSparePart.getSparePartsCategoriesDetails());
                }

            }
            String[] strings = secondSpecials.toArray(new String[secondSpecials.size()]);
            //System.out.println("secondSpecials:" + secondSpecials.toString());
            //System.out.println("suppliers2.get(0).getName():" + suppliers2.get(0).getName());
            try {
                List<PurcSupplierOutput> purcSupplierOutputs = supplierService.getSupplierBySecondSpecial(strings, suppliers2.get(0).getName(), null);
...
*/
    ...
    public Result saveBiddingResult(AddsupplierBiddingInput input) {
        ...
        if (now.getTime() > calendar.getTime().getTime() || now.getTime() < calendar2.getTime().getTime()) {
            return Result.Error(ResultCodeEnum.ERROR.getCode(),"不在竞价时间范围内");
        }
        ...
        if (supplierTenders == null || supplierTenders.size() == 0) {
            ...
            supplierTender.setCreateUser(getCurrentUserId());
            supplierTender.setCreateTime(new Date());
            System.out.println("supplierTender.getBiddingNum():" + supplierTender.getBiddingNum());
            int i = supplierTenderMapper.insertSelective(supplierTender);
    ...
    public Result supplierSign(Integer pureMatterPurchaseId) {
        ...
        if (matterTenderPurchases.get(0).getFirstInstanceTime() != null) {
            if (dateCompare1(now1, matterTenderPurchases.get(0).getFirstInstanceTime()) > 0) {
                return Result.Error(ResultCodeEnum.ERROR.getCode(),"签到时间已经截止");
            }
        }else {
            if (dateCompare(now1,matterTenderPurchases.get(0).getBiddingStartTime())>0){
                return Result.Error(ResultCodeEnum.ERROR.getCode(),"签到时间已经截止");
            }
    ...
    @Override
    public MatterSupplierTenderDetailOutput getPeriodSupplierTenderList(Integer purcMatterTenderPurchaseId) {
        //报价商品
        SupplierTenderProductExample example = new SupplierTenderProductExample();
        SupplierTenderProductExample.Criteria criteria = example.createCriteria();
        ...
        MatterSupplierTenderDetailOutput output = new MatterSupplierTenderDetailOutput();
        BeanUtils.copyProperties(mtp, output);
        System.out.println("mtp:" + mtp.getBiddingStartTime());
        output.setInvoiceLangLang(mtp.getInvoice());
        output.setIsPeriodLangLang(mtp.getIsPeriod());
        output.setPurchaseCompanyLangLang(mtp.getPurchaseCompany());
        output.setSelectSupplierLangLang(mtp.getSelectSupplier());
    ...
    @Override
    public void addPeriodBiddingResult(AddsupplierBiddingInput input) {
