...
import com.sanlly.finance.service.BaseTaxRateService;
 ...
 */
@Service
public class BaseTaxRateServiceImpl extends BaseServiceImpl implements BaseTaxRateService {

	@Autowired
	private BaseTaxRateMapper dao;
	 ...
	 */
	@Override
	public List<BaseTaxRateOutput> list(SearchBaseInput input) {
		BaseTaxRateExample example = new BaseTaxRateExample();
		BaseTaxRateExample.Criteria criteria = example.createCriteria();
		// 过滤掉删除的记录
		criteria.andIsDelEqualTo(PlatformConstants.ISDEL_NO)
				.andIsEnabledEqualTo(FinanceIsEnabledEnum.Enabled.getCode());
		example.setOrderByClause(" `create_time` DESC");
		List<BaseTaxRate> list = dao.selectByExample(example);
		List<BaseTaxRateOutput> outputList = list.stream().map(a -> {// 创建日期倒序排列
			BaseTaxRateOutput output = new BaseTaxRateOutput();
			// 复制属性值到新对象
	 ...
	 */
	@Override
	public PagedList<BaseTaxRateOutput> getPage(SearchBaseInput input) {
		...
		PagedList<BaseTaxRateOutput> pagedList = new PagedList<BaseTaxRateOutput>(input.getPageIndex(),
	 ...
	 */
	@Override
	public Boolean addOrUpadte(BaseTaxRateInput input) {
		// TODO Auto-generated method stub
		BaseTaxRate record = new BaseTaxRate();
		...
		if (StringUtils.isEmpty(input.getId())) {
			...
			// 查询一下信息
			BaseTaxRate info = dao.selectByPrimaryKey(input.getId());
	...
	public Boolean deletes(String[] ids) {
		...
		for (int i = 0; i < ids.length; i++) {
			BaseTaxRate record = dao.selectByPrimaryKey(Integer.parseInt(ids[i]));
