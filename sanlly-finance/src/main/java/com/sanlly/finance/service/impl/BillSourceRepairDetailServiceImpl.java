...
import com.sanlly.finance.service.BillSourceRepairDetailService;
 ...
 */
@Service
public class BillSourceRepairDetailServiceImpl extends BaseServiceImpl implements BillSourceRepairDetailService {

	@Autowired
	private BillSourceRepairDetailMapper dao;
	 ...
	 */
	@Override
	public PagedList<BillSourceRepairDetailOutput> getPage(BillDetailInput input) {
		...
		PagedList<BillSourceRepairDetailOutput> pagedList = new PagedList<BillSourceRepairDetailOutput>(input.getPageIndex(), input.getPageSize(),
				totalCount);
		// 设置开始行数
		...
		List<BillSourceRepairDetail> list = dao.selectByExample(example);
	 ...
	 */
	@Override
	public Boolean addOrUpadte(BillSourceRepairDetailInput input) {
		// TODO Auto-generated method stub
		...
		if (StringUtils.isEmpty(input.getBillId())) {
			...
			// 查询一下信息
			BillSourceRepairDetail info = dao.selectByPrimaryKey(input.getBillId());
	...
	public Boolean deletes(String[] ids) {
		...
		for (int i = 0; i < ids.length; i++) {
			BillSourceRepairDetail record = dao.selectByPrimaryKey(Integer.parseInt(ids[i]));
